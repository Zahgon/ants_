...
<b>A worker-thread pool for Rust</b>
<br/><br/>
...
Crate `ants` implements a worker pool with fixed capacity, managing and recycling a massive number of worker threads, allowing developers to limit the number of live threads in your concurrent programs.

## 🚀 Features:

- Managing and recycling a massive number of worker threads automatically
- Purging overdue workers periodically
- Abundant APIs: submitting tasks, getting the number of running workers, tuning the capacity of the pool dynamically, releasing the pool, rebooting the pool, etc.
- Handle panics gracefully to prevent programs from crashing
- Efficient in memory usage and it may even achieve ***higher performance*** than spawning an unbounded number of threads in Rust
...
`ants` builds on stable Rust 1.75 or newer and pulls in no dependency beyond
`chrono`, which it uses only to render the default logger's timestamps.

## 🛠 How to use
Check out [the examples](https://docs.rs/ants) for basic usage.

### Functional options for pool

`ants::Options` contains all optional configurations of the ants pool, which allows you to customize the worker pool by passing option functions that set up each configuration to `Pool::new`/`PoolWithFunc::new`/`PoolWithFuncGeneric::new`.

Rust has no variadic arguments, so where the original takes `options ...Option` the port takes a slice of `ants::Opt`, and the option type is named `Opt` so it cannot shadow `std::option::Option`.

### Customize pool capacity

`ants` supports customizing the capacity of the pool. You can call `Pool::new` to instantiate a `Pool` with a given capacity, as follows:
...
You can tune the capacity of an `ants` pool at runtime with `Pool::tune`:
...
Don't worry about the contention problems in this case, the method here is thread-safe.

### Pre-malloc worker queue in pool

`ants` allows you to pre-allocate the memory of the worker queue in the pool, which may get a performance enhancement under some special certain circumstances such as the scenario that requires a pool with ultra-large capacity, meanwhile, each task lasts for a long time, in this case, pre-mallocing will reduce a lot of memory allocation in the worker queue.

```rust
// ants will pre-malloc the whole capacity of the pool when calling Pool::new.
...
```rust
// A pool that has been released can still be used after calling reboot().
pool.reboot();
```

## ⚙️ About sequence

All tasks submitted to an `ants` pool will not be guaranteed to be addressed in order, because those tasks scatter among a series of concurrent workers, thus those tasks would be executed concurrently.
